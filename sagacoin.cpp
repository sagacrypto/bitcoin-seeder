...
#define SAGACOIN_SEED_NONCE  0x0539a019ca550825ULL
...
//    printf("%s: SEND %s\n", ToString(you).c_str(), pszCommand); 
  ...
  void PushVersion() {
    ...
    uint64 nLocalNonce = SAGACOIN_SEED_NONCE;
    int64 nLocalServices = 0;
    CAddress me(CService("0.0.0.0"));
    BeginMessage("version");
    int nBestHeight = GetRequireHeight();
    string ver = "/sagacoin-seeder:0.01/";
...
/*
int main(void) {
  CService ip("sagacoin.sipa.be", 8333, true);
  vector<CAddress> vAddr;
  vAddr.clear();
  int ban = 0;
